(function(){
  ...
  function render(reviews){
    ...
    reviews.slice(0,4).forEach(function(r){
      ...
      const imgTag = avatar ? '<img class="avatar" src="' + avatar + '" alt="" onerror="this.style.display=\'none\'">' : '';
      const authorHTML = '<div class="author">' + imgTag + escapeHtml(author) + '</div>';
      const snippetHTML = '<div class="snippet">' + escapeHtml(text) + '</div>';
      const metaHTML = '<div class="meta">' + escapeHtml(time) + '</div>';
      el.innerHTML = starsHTML + snippetHTML + authorHTML + metaHTML;
